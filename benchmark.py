...
from utils import downsample, cany_oper, modcrop
...
class Benchmark:
    ...
    def eval(self, sess, g_ypred, log_path=None, iteration=0):
        pred = []
        
        for i, x in enumerate(self.images_lr):
            x = x/255.0

            output = sess.run(g_ypred, \
                feed_dict={'LR_image:0': x[np.newaxis],'training_net:0':False})
